...
1.createElemWithText 
  ...
  g.Usedocument.createElement()to create the requestedHTML element 
...
/*
3.toggleCommentSection a.Receives a postId as the parameter 
  ...
  e.Togglesthe class 'hide' on the section element 
  f.Return the section element
*/
function toggleCommentSection(postId) {
  const section = document.querySelector(`section[data-post-id="${postId}"]`);
  if (!section) {
    console.log(`Section with postId ${postId} not found.`);
    return;
  }
...
5.deleteChildElements 
  ...
  d.Use parentElement.removeChildto remove the childin the loop 
...
6.addButtonListeners 
  ...
  e.Adds a click event listener to each button (referenceaddEventListener) 
...
function removeButtonListeners(){
    ...
    for(let button of buttons) {
        ...
        button.removeEventListener('click', postId);
...
8.createComments 
  ...
  c.Creates a fragment element withdocument.createDocumentFragment() 
  d.Loop through the comments e.For each comment do the following: 
...
11.getUserPosts 
  ...
  b.Fetches post data for a specific user id from: https://jsonplaceholder.typicode.com/(look at Routessection) 
  c.Should be an async function 
  d.Should utilize a try / catch block e.Uses the fetch API to request all posts for a specific user id 
...
12.getUser 
  ...
  c.Should be an async function 
  d.Should utilize a try / catch block e.Uses the fetch API to request a specific user id 
...
13.getPostComments 
  ...
  b.Fetches comments for a specific post id from: https://jsonplaceholder.typicode.com/(look at Routessection) 
  c.Should be an async function 
...
14.displayComments 
  ...
  f.Adds the classes'comments' and 'hide' to the sectionelement 
...
15.createPosts 
  ...
  h.Create an h2 element with the post title i.Create an p element with the post body 
  ...
  o.Set an attribute on the button with button.dataset.postId= post.id p.Append the h2, paragraphs, button, and section elements you have created to the article element. 
...
16.displayPosts 
  ...
  b.Is an async function c.Receives posts data as a parameter 
...
b.Receives 2 parameters: (see addButtonListeners functiondescription) 
